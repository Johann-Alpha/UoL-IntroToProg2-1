	...
	this.draw = function() {
		...
		for (var i = 0; i < this.plotsDown; i++) {
			for (var j = 0; j < this.plotsAcross; j++) {
				...
				// var x =  (i + 1) * gutterWidth + i * (width - 3 * gutterWidth) / this.plotsDown; 
				// var y = (j + 1) * gutterWidth + j * (height - 3 * gutterWidth) / this.plotsAcross;
				// var w = (width - 3 * gutterWidth) / this.plotsDown; 
				// var h = (height - 3 * gutterWidth) / this.plotsAcross;

				var x =  (i + 1) * this.pad + i * (width - (this.plotsAcross + 1) * this.pad) / this.plotsAcross; 
				var y = (j + 1) * this.pad + j * (height - (this.plotsDown + 1) * this.pad) / this.plotsDown;
				var w = (width - (this.plotsAcross + 1) * this.pad) / this.plotsAcross; 
				var h = (height - (this.plotsAcross + 1) * this.pad) / this.plotsDown;

				// var x =  this.pad + i * this.plotWidth; 
				// var y = this.pad + j * plotHeight;
				...
				this.ticks(x + w/2, y + h, this.frequencyBins[i + 2 * j]);
