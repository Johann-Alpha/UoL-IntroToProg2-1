function PayGapByJob2017() {
  ...
  this.draw = function() {
    ...
    for (i = 0; i < this.data.getRowCount(); i++) {
      ...
      // Female proportion to x position on the canvas.
      var x_pos = map(propFemale[i], propFemaleMin, propFemaleMax,
        this.pad, width - this.pad);
      // paygap to y position on the canvas.
      var y_pos = map(payGap[i], payGapMin, payGapMax,
        height - this.pad, this.pad);
      // number of jobs to dot size.
      var dotSize = map(numJobs[i], numJobsMin, numJobsMax,
        this.dotSizeMin, this.dotSizeMax);
      
      ellipse(x_pos, y_pos, dotSize);
    }
  };
