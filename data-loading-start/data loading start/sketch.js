...
function setup() {
	...
	// find the surname column (or any other column) for the selected row
	console.log(firstXander.getString("surname"));


	// to get values in a column
	...
	// let's continue with the example in the video

	myScatter.setupAxis(0, 100, 0, 100, 10, 10);

	myScatter.addDataPoints(quizMarks, projectMarks, 3, "red");

	myScatter.lineOfBestFit(quizMarks, projectMarks, "blue"); // remember that the functions return strings, so we need to parse the values to get numbers, otherwise this will be NaN.
